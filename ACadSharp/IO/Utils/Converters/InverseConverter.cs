...
namespace ACadSharp.IO.Utils.Converters
{
	internal class InverseConverter : IEndianConverter
	{
		...
		public short ToInt16(byte[] arr, int offset) => BitConverter.ToInt16(fullInverse(arr, offset), 0);
		public ushort ToUInt16(byte[] arr, int offset) => BitConverter.ToUInt16(fullInverse(arr, offset), 0);
		public int ToInt32(byte[] arr, int offset) => BitConverter.ToInt32(fullInverse(arr, offset), 0);
		public uint ToUInt32(byte[] arr, int offset) => BitConverter.ToUInt32(fullInverse(arr, offset), 0);
		public long ToInt64(byte[] arr, int offset) => BitConverter.ToInt64(fullInverse(arr, offset), 0);
		public ulong ToUInt64(byte[] arr, int offset) => BitConverter.ToUInt64(fullInverse(arr, offset), 0);
		public double ToDouble(byte[] arr, int offset) => BitConverter.ToDouble(fullInverse(arr, offset), 0);
		public float ToSingle(byte[] arr, int offset) => BitConverter.ToSingle(fullInverse(arr, offset), 0);
