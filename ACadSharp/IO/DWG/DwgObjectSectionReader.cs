...
namespace ACadSharp.IO.DWG
{
	...
	internal class DwgObjectSectionReader : DwgSectionReader
	{
			...
			DxfClassCollection classes) : base(builder.DocumentToBuild.Header.Version)
		{
			...
			_buffer = new byte[_crcStream.Length];
			this._crcStream.Read(_buffer, 0, _buffer.Length);
			_crcReader.Position = 0;
		}
		...
		private ObjectType getEntityType(long offset)
		{
			...
			if (this.R2010Plus)
			{
				...
				this._objectReader = DwgStreamReader.GetStreamHandler(this._version, new MemoryStream(_buffer));
				...
				//Create a handler section reader
				this._handlesReader = DwgStreamReader.GetStreamHandler(this._version, new MemoryStream(_buffer));
				this._handlesReader.SetPositionInBits((long)handleSectionOffset);

				//Create a text section reader
				this._textReader = DwgStreamReader.GetStreamHandler(this._version, new MemoryStream(_buffer));
