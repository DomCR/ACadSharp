...
namespace ACadSharp.IO.DWG
{
	...
	internal static class DwgAcDsDataWriter
	{
		...
		//fixed schema segments: the schema definitions do not depend on the
		//stored records.
		//canonical schema, byte for byte as ODA and AutoCAD emit it (identical in
		//both, verified across record counts): six schemas with AcDb3DSolid_ASM_Data
		//at index 1 (AcDb_Thumbnail_Schema is index 0). The segment number field is
		//patched from 6 (its native value) to 4 to fit this writer's segment layout.
		private const string SCHIDX_HEX =
			"";
		...
		private static byte[] buildDatidxSegment(int count)
		{
			...
			using (BinaryWriter writer = new BinaryWriter(stream))
			{
				...
				for (int i = 0; i < count; i++)
				{
					...
					writer.Write(i * 20);
					writer.Write(1);   //schema index: AcDb3DSolid_ASM_Data is schema 1
				}
