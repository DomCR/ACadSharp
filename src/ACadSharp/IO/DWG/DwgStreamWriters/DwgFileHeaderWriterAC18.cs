...
internal class DwgFileHeaderWriterAC18 : DwgFileHeaderWriterBase<DwgFileHeaderAC18>
{
	public override int FileHeaderSize { get { return 0x100; } }
	...
	protected void writeFileMetaData()
	{
		...
		//0x13	2	Codepage
		writer.Write(this.getFileCodePage());
		//0x15	3	3 0x00 bytes
	...
	private void writeFileHeader(MemoryStream stream)
	{
		...
		swriter.Write(this.FileHeader.RootTreeNodeGap);
		//0x1C	4	Lowermost left tree node gap
		swriter.Write(this.FileHeader.LeftGap);
		//0x20	4	Lowermost right tree node gap
		swriter.Write(this.FileHeader.RigthGap);
		//0x24	4	Unknown long(ODA writes 1)
		swriter.Write(1);
		//0x28	4	Last section page Id
		swriter.Write(this.FileHeader.LastPageId);

		//0x2C	8	Last section page end address
		swriter.Write(this.FileHeader.LastSectionAddr);
		//0x34	8	Second header data address pointing to the repeated header data at the end of the file
		swriter.Write(this.FileHeader.SecondHeaderAddr);

		//0x3C	4	Gap amount
		swriter.Write(this.FileHeader.GapAmount);
		//0x40	4	Section page amount
		swriter.Write(this.FileHeader.SectionAmount);
		...
		swriter.Write(this.FileHeader.PageMapAddress - 256);
		//0x5C	4	Section Map Id
		swriter.Write(this.FileHeader.SectionMapId);
		//0x60	4	Section page array size
		swriter.Write(this.FileHeader.SectionArrayPageSize);
		//0x64	4	Gap array size
		swriter.Write(this.FileHeader.GapArraySize);
